...
void G4ParticleHPMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
   ...
   if ( command == CASCADECmd ) { 
      if(bValue == 1){
        G4String temp = "G4NEUTRONHP_USE_CASCADE=1";
	char* c_temp = temp.data();
	putenv(c_temp);
      }else {
        G4String temp = "G4NEUTRONHP_USE_CASCADE=0";
	char* c_temp = temp.data();
	putenv(c_temp);
      }
   }
   if( command == UseRawExcitationCmd ) { 
      if(bValue == 1){
        G4String temp = "G4NEUTRONHP_USE_RAW_EXCITATION=1";
	char* c_temp = temp.data();
	putenv(c_temp);
      }else {
        G4String temp = "G4NEUTRONHP_USE_RAW_EXCITATION=0";
	char* c_temp = temp.data();
	putenv(c_temp);
      }
   }
   if( command == DoUnplacedCmd ) { 
      if(bValue == 1){
        G4String temp = "G4NEUTRONHP_DO_UNPLACED=1";
	char* c_temp = temp.data();
	putenv(c_temp);
      }else {
        G4String temp = "G4NEUTRONHP_DO_UNPLACED=0";
	char* c_temp = temp.data();
	putenv(c_temp);
      }
   }
